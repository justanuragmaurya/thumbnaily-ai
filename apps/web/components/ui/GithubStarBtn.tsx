...
const GithubStarBtn = () => {
    ...
    const username = 'justanuragmaurya';
    const repo = 'thumbnaily-ai';
    ...
    useEffect(() => {
        ...
        const fetchStarCount = async () => {
            try {
                ...
                setStarCount(data.stargazers_count || <ExternalLink size={18} />);
            } catch (error) {
    ...
    return (
        <Link href={`https://github.com/${username}/${repo}`} target='_blank' rel='noreferrer noopener'>
            <button className="flex cursor-pointer overflow-hidden items-center text-sm font-medium focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 bg-black text-white shadow hover:bg-black/90 h-9 px-4 py-2 max-w-52 whitespace-pre md:flex group relative w-full justify-center gap-2 rounded-md transition-all duration-300 ease-out hover:ring-2 hover:ring-black hover:ring-offset-2">
                ...
                <div className="ml-2 flex items-center gap-1 text-sm md:flex">
                    ...
                    <span className="inline-block tabular-nums tracking-wider font-display font-medium text-white"> {starCount} </span>
                </div>
            </button>
        </Link>
    );
